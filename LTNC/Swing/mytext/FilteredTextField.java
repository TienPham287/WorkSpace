...
public class FilteredTextField extends JTextField {
  ...
  class MyFilter extends DocumentFilter{
    public void insertString(DocumentFilter.FilterBypass fb, int offset,
         ...
         if ((j>0) && (offset>=defaultLenght)) fb.insertString(offset,new String(result), attr);
    }
  ...
  //    if (fb.getDocument().getLength()>defaultLenght) fb.remove(offset,1);
      if (offset>=defaultLenght) fb.remove(offset,length);
    }
    public void replace(DocumentFilter.FilterBypass fb, int offset, int length,
      ...
      if ((j>0) && (offset>=defaultLenght)) fb.replace(offset,length,new String(result),attrs);
    }
